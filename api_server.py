...
class CertificateVerificationAPI:
    ...
    def _create_app(self) -> FastAPI:
        ...
        app.get("/user/{user_name}/field_stats")(self.get_user_field_stats)
    ...
    async def verify_proof(
        ...
        time_unit: Optional[str] = Form('hours'),
        ...
        try:
            ...
            # Calculate time if provided
            if time_spent is not None:
                # Convert months to hours (1 month = 1 hour)
                hours = time_spent if time_unit == 'hours' else time_spent * 1  # 1 month = 1 hour
                ...
                equivalent_time = hours * multiplier
                
                # Add time information to result (in hours)
                result['original_time'] = hours  # Store in hours
                result['calculated_time'] = equivalent_time  # Store in hours
                result['time_unit'] = time_unit
                result['multiplier'] = multiplier
            
            # Add field to result if provided
            if field:
                result['field'] = field
    ...
    async def get_user_stats(self, user_name: str):
        ...
        try:
            # Get field stats for the user
            field_stats = self.verification_service.get_user_field_stats(user_name)
            return field_stats
        except Exception as e:
            logger.error(f"Error getting user stats: {str(e)}")
            raise HTTPException(status_code=500, detail=str(e))

    async def get_user_field_stats(self, user_name: str):
        ...
        try:
            ...
            logger.error(f"Error getting user field stats: {str(e)}")
            raise HTTPException(status_code=500, detail=str(e))
    ...
    async def submit_all_activities(self, activities: List[Dict] = Body(...)):
        ...
        try:
            ...
            for activity in activities:
                ...
                # Update verification result with time information (in hours)
                verification['original_time'] = hours  # Store in hours
                verification['calculated_time'] = equivalent_time  # Store in hours
                verification['time_unit'] = time_unit
                verification['multiplier'] = multiplier
                ...
                processed_activity = {
                    **activity,
                    ...
                    'original_time': hours,  # Store in hours
                    'calculated_time': equivalent_time  # Store in hours
                }
            ...
            # Update verification status and time information in database
            for activity in processed_activities:
                verification_id = activity['verification_id']
                try:
                    with sqlite3.connect(self.verification_service.db.db_path) as conn:
                        cursor = conn.cursor()
                        # Update submission with status and time information
                        cursor.execute("""
                            UPDATE submissions 
                        ...
                        """, (
                            'submitted',
                            activity['original_time'],
                            activity['calculated_time'],
                            activity['time_unit'],
                            activity['multiplier'],
                            verification_id
                        ))
                    ...
                    logger.error(f"Error updating submission information: {str(e)}")
                    raise
